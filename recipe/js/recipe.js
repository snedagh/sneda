...
function loadMenuItems()
{
    ...
    if(menu_count > 0 )
    {
        ...
        for(let mi = 0; mi < menu_items.length; mi++) // get buttons
        {
            ...
            buttons_html += "<button onclick='get_recipe("+item.barcode+")' class=\"btn btn-danger m-1 recipe_card\">"+ item.item_des +"</button>";
        }
...
function get_recipe(barcode) {
    ...
    if(rec_count < 1)
    {
        ...
        let rec_items = fetch_return('smdesk',"SELECT * FROM recipe WHERE `parent` = '"+barcode+"'")
        cl(rec_items)
        for (let i = 0; i < rec_items.length; i++)
        {
