    ...
    if($_SERVER['REQUEST_METHOD'] === 'GET')
    {
        ...
        elseif(isset($_GET['pagination']))
        {
            ...
            elseif ($direction === 'previous')
            {
                ...
                //get item with limit
                $sql = "SELECT `first_name` FROM `loyalty_new` WHERE `first_name` < '$last' ORDER BY `first_name` DESC LIMIT $mult_limit";
        ...
        // set order
        elseif(isset($_GET['sort']) && isset($_GET['col']))
        {
            $column = $_GET['col'];
            $sort = $_GET['sort'];
        ...
        {
            $file = get('file');
            $ac_file = "../../assets/storage/admin_panel/issues/$file";
            download($ac_file);
        }
        ...
        elseif (isset($_GET['del']) && !empty($_GET['del']))
        {
            ...
            {
                $task = get('del');
                $files = fetchFunc('task', "`id` = '$task'",$pdo)['files'];
                if(deleteRow('task', "`id` = '$task'", $pdo))
                {
                    // delete files

                    foreach (explode('/',$files) as $key => $value)
                    {
                        // delete file
                        $file = "../../assets/storage/admin_panel/issues/$value";
                        delete_file($file);
            ...
            elseif ($tool == 'user_management')
            {
                ...
                $user = get('del');
                deleteRow('users',"`id` = $user",$pdo);

                // drop table
                $apps_table = "user_".$user."_apps";
                drop_table($apps_table,$pdo);
            }
